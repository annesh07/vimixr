ELBO_function <- function(type, params){
  ...
  #the variationa distributions for alpha and z's
  ...
  if (type == "fixed-scalar"){
    ...
    e3 <- sum(P*(-0.5*D*log(2*pi) - 0.5*determinant(C0, logarithm=TRUE)$modulus))
    + sum(e30) + sum(e31) + sum(e32) + sum(e33)

    #the variationa distribution
    e4 <- sum(D*log(L2)/2) - 0.5*D*T0*(log(2*pi)+1) + ev
