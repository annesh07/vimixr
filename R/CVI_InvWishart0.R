CVI_InvWishart0 <- function(N, D, T0, C00, nu0, Lamda0, k0, Lamda, k,
                            X, L1, L2, nu, V, Plog, maxit){
  ...
  f <- list()
  f[[1]] <- ELBO_InvWishart0(N, D, T0, C00, nu0, Lamda0, k0, Lamda, k,
                             X, L1, L2, nu, V, Plog)
  ...
  for (m in 1:maxit){
    ...
    for (n in 1:N){
      ...
      P20 <- log(1 + p_eni) - p_vni/((1 + p_eni)^2) - log(1 + p_eni + p_enj +
                                                            0.4) + (p_vni + p_vnj)/((1 + p_eni + p_enj + 0.4)^2)

      P21 <- log(0.4 + p_enj) - (p_vnj)/((0.4 + p_enj)^2) -
        log(1 + p_eni + p_enj + 0.4) +
        (p_vni + p_vnj)/((1 + p_eni + p_enj + 0.4)^2)
    ...
    f[[m+1]] <- ELBO_InvWishart0(N, D, T0, C00, nu0, Lamda0, k0, Lamda, k,
                                 X, L1, L2, nu, V, Plog)
  ...
  posterior <- list("degrees of freedom"=dof,
