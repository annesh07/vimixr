CVI_C0_i_sparse_N <- function(N, D, T0, s1, s2, Mu0, a0, b0, k0, X,
  ...
  for (i in 1:T0){
    ...
    C01 <- 0.5*(crossprod(sweep(X, 1, P0[,i], "*"), X))
    C1[,,i] <- Diag.fill(C01, rep(0, D))
  }
  f <- list()
  f[[1]] <- ELBO_C0_i_sparse_N(N, D, T0, s1, s2, Mu0, a0, b0, k0, X,
  ...
  for (m in 1:maxit){
    ...
    for (i in 1:T0){
      ...
      C01 <- 0.5*(crossprod(sweep(X, 1, Pf0[,i], "*"), X))
      C1[,,i] <- Diag.fill(C01, rep(0, D))
    ...
    f[[m+1]] <- ELBO_C0_i_sparse_N(N, D, T0, s1, s2, Mu0, a0, b0, k0, X,
                                 W1, W2, L1, a1, B1, C1, Plog)
