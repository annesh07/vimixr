ELBO_C0_nu0 <- function(N, D, T0, s1, s2, C00, b10, b20, V0, X,
  ...
  #the nu0
  e5 <- b10*log(b20) - lgamma(b10) + (b10 - 1)*(digamma(b10)-log(b2)) -
    b20*(b10/b2)

  #the variationa distributions
  e60 <- W1*log(W2) - lgamma(W1) + (W1-1)*(-log(W2) + digamma(W1)) - W1
  e61 <- sum(exp(Plog)*Plog)
  e62 <- sum(apply(L2, 3,
                   function(x){0.5*determinant(x, logarithm = TRUE)$modulus}))
  - 0.5*D*T0*(log(2*pi)+1)
  inv_V <- solve(V)
  e63 <- -0.5*nu*D*log(2) - 0.25*D*(D-1)*log(pi)
  ...
  e64 <- b10*log(b2) - lgamma(b10) + (b10 - 1)*(digamma(b10)-log(b2)) - b10
  e6 <- e60 + e61 + e62 + e63 + e64
